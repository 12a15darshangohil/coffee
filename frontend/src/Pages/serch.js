...
const Serch = () =>{
    ...
    useEffect(async ()=>{
        ...
        let Data = []
        await fetch("http://127.0.0.1:8000/api/drink/")
        .then(res => res.json())
        ...
        await fetch("http://127.0.0.1:8000/api/merchandise/")
        .then(res => res.json())
        .then( data => data.map(item =>{
             Data.push(item)
        }))
        await fetch("http://127.0.0.1:8000/api/coffee-at-home/")
        .then(res => res.json())
        .then( data => data.map(item =>{
             Data.push(item)
        }))
        await fetch("http://127.0.0.1:8000/api/ready-to-eat/")
        .then(res => res.json())
        .then( data => data.map(item =>{
             Data.push(item)
        }))
